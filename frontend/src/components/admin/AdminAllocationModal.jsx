...
export default function AdminAllocationModal({ instrument, onSave, onCancel }) {
  const [rows, setRows] = useState(
    instrument.countryAllocation?.length
      ? instrument.countryAllocation.map(a => ({ country: a.country, percentage: String(a.percentage) }))
      : [{ country: '', percentage: '' }]
  )
  ...
  function addRow() {
    setRows(rs => [...rs, { country: '', percentage: '' }])
  }
  ...
  async function handleSave() {
    const entries = rows
      ...
      .map(r => ({ country: r.country.trim(), percentage: parseFloat(r.percentage) || 0 }))

    setSaving(true)
    setError(null)
    try {
      const saved = await updateInstrumentAllocations(instrument.id, entries)
    ...
    <div className="fixed inset-0 bg-black/40 flex items-end sm:items-center justify-center z-60">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-bold text-gray-900">Allocation géographique</h2>
