...
// Convertit une ISO UTC stockée par le backend en "YYYY-MM-DDTHH:mm" heure locale
// pour pré-remplir un DateTimeInput (qui attend de l'heure locale)
function utcIsoToLocalInput(iso) {
  if (!iso) return ''
  const d = new Date(iso + 'Z') // force le parse en UTC
...
function DetailPanel({ bug, detailLoading, onClose, onUpdated, onNavigate, onDelete }) {
  ...
  function startEdit() {
    setEditForm({
      ...
      approximateDateTime: utcIsoToLocalInput(bug.approximateDateTime),
      userImpact:          bug.userImpact ?? 'MEDIUM',
    })
    setEditError(null)
    setEditing(true)
  }
  ...
  async function handleEditSave(e) {
    ...
    setEditError(null)
    try {
      const updated = await updateAdminBugReport(bug.id, {
        title:               editForm.title.trim(),
        description:         editForm.description.trim(),
        expectedResult:      editForm.expectedResult.trim() || null,
        reproductionSteps:   editForm.reproductionSteps.trim() || null,
        approximateDateTime: editForm.approximateDateTime
          ? new Date(editForm.approximateDateTime).toISOString().slice(0, 19)
          : null,
        userImpact: editForm.userImpact,
      })
      onUpdated(updated)
      setEditing(false)
    } catch (err) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-5 space-y-5">
      ...
      {editing ? (
        <form onSubmit={handleEditSave} className="space-y-4">
          <div>
            ...
            <input type="text" required maxLength={200} value={editForm.title}
          ...
          <div>
            ...
            <textarea required maxLength={3000} rows={4} value={editForm.description}
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">Résultat souhaité</label>
            <textarea maxLength={2000} rows={2} value={editForm.expectedResult}
              onChange={e => setEditForm(f => ({ ...f, expectedResult: e.target.value }))}
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-y"
            />
          </div>
          <div>
            <label className="block text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">Étapes de reproduction</label>
            <textarea maxLength={3000} rows={3} value={editForm.reproductionSteps}
              onChange={e => setEditForm(f => ({ ...f, reproductionSteps: e.target.value }))}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">Date et heure approximatives</label>
              <DateTimeInput
                value={editForm.approximateDateTime}
                onChange={v => setEditForm(f => ({ ...f, approximateDateTime: v }))}
              />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-500 mb-2 uppercase tracking-wide">Impact *</label>
              <select value={editForm.userImpact}
                onChange={e => setEditForm(f => ({ ...f, userImpact: e.target.value }))}
                ...
                {IMPACTS.map(i => <option key={i} value={i}>{SEVERITY_LABEL[i]}</option>)}
              </select>
            </div>
          </div>
          {editError && (
        ...
        /* Contenu lecture — affiché immédiatement, placeholder pendant le fetch du détail */
        detailLoading ? (
      ...
      {/* Triage admin + commentaires + logs — masqués en mode édition */}
      {!editing && (
        <>
          ...
          {bug.approximateDateTime && (
            <section>
              <div className="flex items-center justify-between mb-2">
                ...
                {!logs && (
                  <button onClick={handleExtractLogs} disabled={logsLoading}
