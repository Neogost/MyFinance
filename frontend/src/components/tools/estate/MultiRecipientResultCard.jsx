...
export default function MultiRecipientResultCard({ result }) {
  if (!result) return null

  const totalDroits  = parseFloat(result.totalDroitsEur ?? 0)
  const isDismembered = result.recipients?.[0]?.npRatio != null
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {result.recipients.map(r => {
          const droits = parseFloat(r.droitsEur ?? 0)
          return (
            <div key={r.recipientId} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              ...
              <div className="px-4 py-2">
                <Row label="Part allouée" value={`${fmt(r.allocatedAmountEur)} €`} bold />
                {r.npRatio != null && (
                  <>
                    <Row label={`Ratio NP (art. 669)`} value={`${Math.round(parseFloat(r.npRatio) * 100)} %`} indent />
                    <Row label="Valeur fiscale (NP)" value={`${fmt(r.valueTransmittedEur)} €`} indent bold />
                  </>
                )}
                <Row label="Abattement légal" value={`${fmt(r.abattementBaseEur)} €`} bold />
                {parseFloat(r.abattementUsedEur ?? 0) > 0 && (
                  <Row label="Déjà utilisé (donations < 15 ans)"
                    value={`− ${fmt(r.abattementUsedEur)} €`}
                    indent color="text-amber-600 dark:text-amber-400" />
                )}
                <Row label="Abattement disponible"
                  value={`${fmt(r.abattementResiduelEur)} €`}
                  indent color="text-emerald-600 dark:text-emerald-400" />
                <Row label="Part taxable"
                  value={`${fmt(r.taxableEur)} €`}
                  bold color={droits > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-800'} />
                <Row label="Droits à payer"
                  value={`${fmt(r.droitsEur)} €`}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
          <p className="text-sm font-semibold text-gray-700">Total — donation à {result.recipients.length} bénéficiaires</p>
        </div>
        <div className="px-4 py-2">
          <Row label="Montant total transmis"
            value={`${fmt(result.totalAmountGivenEur)} €`} bold color="text-gray-800" />
          <Row label="Droits totaux (cumulés sur tous les bénéficiaires)"
        ...
        <div className="px-4 py-2.5 border-t border-gray-100 text-xs text-gray-400 bg-gray-50 space-y-0.5">
          ...
          <p>Chaque bénéficiaire bénéficie de son propre abattement (cumulables).</p>
